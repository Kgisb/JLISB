...
def _render_performance_deal_stage(
    ...
    d["_last_only_date"] = d["_last"].dt.date
    last_status = d.groupby(group_cols, dropna=False)["_last_only_date"].max().reset_index(name="Last Activity Date (max)")
    last_status["Days Since Last Act"] = (today - last_status["Last Activity Date (max)"]).dt.days
