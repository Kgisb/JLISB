...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Activity tracer", "Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity"],
...
def render_activity_tracer(df_scope: pd.DataFrame):
    ...
    import altair as alt
    from datetime import date
    # Expect these globals to be defined earlier in the app:
    global dealstage_col, create_col, last_activity_col
    global num_times_contact_col, num_sales_act_col
    global counsellor_col, country_col, source_col
    ...
    # Date range on Last Activity Date
    def month_bounds(d: date):
        from calendar import monthrange
        start = date(d.year, d.month, 1)
        end = date(d.year, d.month, monthrange(d.year, d.month)[1])
    ...
    if num_times_contact_col and num_times_contact_col in df_scope.columns:
        agg_choices.append("Sum: Number of times contacted")
    if num_sales_act_col and num_sales_act_col in df_scope.columns:
        agg_choices.append("Sum: Number of Sales Activities")
    ...
    # Primary dimension to show composition per row
    prim_opts = []
    if counsellor_col and counsellor_col in df_scope.columns: prim_opts.append("Academic Counsellor")
    if country_col and country_col in df_scope.columns:       prim_opts.append("Country")
    if source_col and source_col in df_scope.columns:         prim_opts.append("JetLearn Deal Source")
    if not prim_opts:
    ...
    d["_ntc"] = pd.to_numeric(d[num_times_contact_col], errors="coerce").fillna(0).astype(int) if (num_times_contact_col and num_times_contact_col in d.columns) else 0
    d["_nsa"] = pd.to_numeric(d[num_sales_act_col], errors="coerce").fillna(0).astype(int) if (num_sales_act_col and num_sales_act_col in d.columns) else 0

    mask_date = (d["_last"] >= start_d) & (d["_last"] <= end_d)
    ...
    if mode == "MTD":
        ...
        mask_mtd = (d["_create"] >= ms) & (d["_create"] <= me)
    ...
    if counsellor_col and counsellor_col in d.columns:
        d["Academic Counsellor"] = d[counsellor_col].fillna("Unknown").astype(str)
    if country_col and country_col in d.columns:
        d["Country"] = d[country_col].fillna("Unknown").astype(str)
    if source_col and source_col in d.columns:
        d["JetLearn Deal Source"] = d[source_col].fillna("Unknown").astype(str)

    # Aggregation value
    if agg_pick == "Deals (count)":
        d["_val"] = 1; agg_title = "Deals (count)"
    elif agg_pick == "Sum: Number of times contacted" and (num_times_contact_col and num_times_contact_col in d.columns):
        d["_val"] = d["_ntc"]; agg_title = "Sum of Number of times contacted"
    elif agg_pick == "Sum: Number of Sales Activities" and (num_sales_act_col and num_sales_act_col in d.columns):
        d["_val"] = d["_nsa"]; agg_title = "Sum of Number of Sales Activities"
    else:
        d["_val"] = 1; agg_title = "Deals (count)"

    # Composition by Deal Stage
    ...
    st.caption(
        ...
        f"• MTD includes only rows where **Create Date** is within the month of the range end."
    )
