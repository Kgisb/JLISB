        ...
        if long_df.empty:
            ...
            if agg_view == "Month-on-Month":
                ...
                if view_mode == "Graph":
                    ...
                    st.dataframe(pivot if "pivot" in locals() else (tbl if "tbl" in locals() else (grp if "grp" in locals() else df.copy().head(0))), use_container_width=True, hide_index=True)
csv = (pivot if "pivot" in locals() else (tbl if "tbl" in locals() else (grp if "grp" in locals() else df.copy().head(0)))).to_csv(index=False).encode("utf-8")
