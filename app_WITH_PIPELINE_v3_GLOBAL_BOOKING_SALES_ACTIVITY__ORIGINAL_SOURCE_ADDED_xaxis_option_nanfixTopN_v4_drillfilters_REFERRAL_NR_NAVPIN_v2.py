...
def _render_performance_referral_no_referral(
    ...
    st.caption("Window A (left) defaults to **Referral**; Window B (right) defaults to **Non-Referral**.")
    ...
    def _agg_side(d0, start_d, end_d, src_pick):
        ...
        # Referral generated = Referral Intent Source contains "Sales generated" (on created in window)
        if "_refi" in dd:
            ref_gen = int((m_create & dd["_refi"].str.contains(r"\\bsales\\s*generated\\b", case=False, na=False, regex=True)).sum())
            # Keep a separate self_gen metric (same definition unless you want a different one)
            self_gen = ref_gen
        else:
            ref_gen = 0
            self_gen = 0
    ...
    st.divider()
    _kpi_row("Deals created", resA["Deals"], resB["Deals"])
    _kpi_row("Enrollments", resA["Enrolments"], resB["Enrolments"])
    _kpi_row("Trial Scheduled", resA["FirstCal"], resB["FirstCal"])
    _kpi_row("Trial Rescheduled", resA["CalResch"], resB["CalResch"])
    _kpi_row("Calibration Done", resA["CalDone"], resB["CalDone"])
    _kpi_row("Referral generated (Referral Intent Source=Sales generated)", resA["ReferralGenerated"], resB["ReferralGenerated"])
    _kpi_row("Self-generated referrals (Referral Intent Source=Sales generated)", resA["SelfGenReferrals"], resB["SelfGenReferrals"])
    _kpi_row("Lead → Enrollment %", resA["L2E_pct"], resB["L2E_pct"], suffix="%")
    ...
    chart_df = pd.DataFrame([
        ...
        {"Metric":"Referral generated (Referral Intent Source=Sales generated)","Window":"B (Non-Referral)","Value":resB["ReferralGenerated"]},
    ...
    tbl = pd.DataFrame([
        {"Side":"A (Referral)",
         ...
         "Referral generated (Referral Intent Source=Sales generated)":resA["ReferralGenerated"],
        ...
        {"Side":"B (Non-Referral)",
         ...
         "Referral generated (Referral Intent Source=Sales generated)":resB["ReferralGenerated"],
...
# Fallback quick open in case a different nav system is used
try:
    import streamlit as st
