...
def _render_performance_original_source(
    ...
    if x_axis == "Date (daily)":
        ...
        # Graph
        chart_df = grp.copy()
        chart_df[level_col] = chart_df[level_col].astype(str).fillna("Unknown")
        ...
        grp[xcol] = grp[xcol].astype(str).fillna("Unknown").replace({'nan': 'Unknown'})
        ...
        # Table + download
        st.dataframe(grp.rename(columns={xcol: "Original Source Category"}), use_container_width=True, hide_index=True)
