...
def _render_performance_referral_no_referral(
    ...
    st.caption("Window A (left) focuses on **Referral**; Window B (right) on **Non‑Referral** by default. Compare counts and Lead→Enrollment%.")

    # ---- column guards
    if not (create_col and create_col in df_f.columns):
        st.error("Create Date column not found."); return
    if not (pay_col and pay_col in df_f.columns):
    ...
    def _pick(df: pd.DataFrame, *cands):
        ...
        low = {c.lower().strip(): c for c in df.columns}
        for c in cands:
            if isinstance(c, str) and c.lower().strip() in low: return low[c.lower().strip()]
        return None

    _first = _pick(df_f, first_cal_sched_col, "First Calibration Scheduled Date", "First calibration scheduled date", "First_Calibration_Scheduled_Date")
    _resch = _pick(df_f, cal_resched_col, "Calibration Rescheduled Date", "Calibration rescheduled date", "Calibration_Rescheduled_Date")
    _done  = _pick(df_f, cal_done_col,  "Calibration Done Date", "Calibration done date", "Calibration_Done_Date")
    _src   = _pick(df_f, source_col, "JetLearn Deal Source", "Deal Source", "Source")
    _refi  = _pick(df_f, "Referral Intent Source", "Referral intent source", "Referral_Intent_Source")
    ...
    # Helper
    today = date.today()
    def _bounds_this_month(d0: date):
        from calendar import monthrange
    ...
    with colA:
        ...
        if presetA == "Today":
            ...
            with ca1: a_start = st.date_input("Start (A)", value=_bounds_this_month(today)[0], key="refnr_start_a")
            with ca2: a_end   = st.date_input("End (A)",   value=today, key="refnr_end_a")
            if a_end < a_start: a_start, a_end = a_end, a_start

        # Referral source filter (always referral by default)
    ...
    with colB:
        ...
        if presetB == "Today":
            ...
            with cb1: b_start = st.date_input("Start (B)", value=_bounds_this_month(today)[0], key="refnr_start_b")
            with cb2: b_end   = st.date_input("End (B)",   value=today, key="refnr_end_b")
            if b_end < b_start: b_start, b_end = b_end, b_start

        # Non‑Referral default (everything except referral), but allow custom pick
    ...
    # Metric helpers
    def _mask_window(d0: pd.DataFrame, start_d: date, end_d: date, *, mode: str):
        m_create = d0["_create"].dt.date.between(start_d, end_d)
        m_pay    = d0["_pay"].dt.date.between(start_d, end_d)
        m_first  = d0["_first"].dt.date.between(start_d, end_d) if "_first" in d0 else pd.Series(False, index=d0.index)
        m_resch  = d0["_resch"].dt.date.between(start_d, end_d) if "_resch" in d0 else pd.Series(False, index=d0.index)
        m_done   = d0["_done"].dt.date.between(start_d, end_d)  if "_done"  in d0 else pd.Series(False, index=d0.index)
        if mode == "MTD":
    ...
    def _agg_side(d0: pd.DataFrame, start_d: date, end_d: date, src_pick: list[str]) -> dict:
        dd = d0[d0["_src"].isin(src_pick)].copy()
        if dd.empty:
            return dict(
                Deals=0, Enrolments=0, FirstCal=0, CalResch=0, CalDone=0,
        ...
        first = int(m_first.sum()) if isinstance(m_first, pd.Series) else 0
        resch = int(m_resch.sum()) if isinstance(m_resch, pd.Series) else 0
        done  = int(m_done.sum())  if isinstance(m_done, pd.Series)  else 0
        # Referral generated (from Deal Source = Referral created in window)
        ref_gen = int((m_create & dd["_src"].str.contains("referr", case=False, na=False)).sum())
        # Self‑generated referrals from Referral Intent Source contains 'sales generated' on CREATED in window
        if "_refi" in dd:
            self_gen = int((m_create & dd["_refi"].str.contains(r"\\bsales\\s*generated\\b", case=False, na=False, regex=True)).sum())
        else:
            self_gen = 0
        l2e = (enrol / deals * 100.0) if deals else np.nan
        return dict(
            Deals=deals, Enrolments=enrol, FirstCal=first, CalResch=resch, CalDone=done,
    ...
    # ----- KPIs side-by-side
    def _kpi_block(title: str, a, b, suffix=""):
    ...
    st.divider()
    _kpi_block("Deals created", resA["Deals"], resB["Deals"])
    _kpi_block("Enrollments", resA["Enrolments"], resB["Enrolments"])
    _kpi_block("Trial Scheduled", resA["FirstCal"], resB["FirstCal"])
    _kpi_block("Trial Rescheduled", resA["CalResch"], resB["CalResch"])
    _kpi_block("Calibration Done", resA["CalDone"], resB["CalDone"])
    _kpi_block("Referral generated (Referral Intent Source=Sales generated)", resA["ReferralGenerated"], resB["ReferralGenerated"])
    _kpi_block("Self‑generated referrals (Referral Intent=Sales generated)", resA["SelfGenReferrals"], resB["SelfGenReferrals"])
    _kpi_block("Lead → Enrollment %", resA["L2E_pct"], resB["L2E_pct"], suffix="%")

    # ----- Chart: stacked bar comparing A vs B for all count metrics
    chart_df = pd.DataFrame([
        ...
        {"Metric":"Referral generated (Referral Intent Source=Sales generated)","Window":"A (Referral)","Value":resA["ReferralGenerated"]},
        {"Metric":"Referral generated","Window":"B (Non‑Referral)","Value":resB["ReferralGenerated"]},
        {"Metric":"Self‑generated referrals","Window":"A (Referral)","Value":resA["SelfGenReferrals"]},
        {"Metric":"Self‑generated referrals","Window":"B (Non‑Referral)","Value":resB["SelfGenReferrals"]},
    ])
    chart = alt.Chart(chart_df).mark_bar().encode(
        x=alt.X("Metric:N", title=None, sort=list(chart_df["Metric"].unique())),
        y=alt.Y("Value:Q", title="Count"),
        column=alt.Column("Window:N", title=None),
        tooltip=[alt.Tooltip("Metric:N"), alt.Tooltip("Window:N"), alt.Tooltip("Value:Q")]
    ...
    tbl = pd.DataFrame([
        {"Side":"A (Referral)",
         ...
         "Referral generated (Referral Intent Source=Sales generated)":resA["ReferralGenerated"], "Self‑generated referrals":resA["SelfGenReferrals"],
        ...
        {"Side":"B (Non‑Referral)",
         ...
         "Referral generated (Referral Intent Source=Sales generated)":resB["ReferralGenerated"], "Self‑generated referrals":resB["SelfGenReferrals"],
         "Lead → Enrollment %": None if np.isnan(resB["L2E_pct"]) else round(resB["L2E_pct"],1)},
    ])
    st.dataframe(tbl, use_container_width=True, hide_index=True)
    st.download_button("Download CSV — Referral vs Non‑Referral (A vs B)", tbl.to_csv(index=False).encode("utf-8"),
                       file_name="referral_vs_nonreferral_A_vs_B.csv", mime="text/csv")

# ---- Register the new pill into the existing navigation (strong, idempotent) ----
try:
    import streamlit as st  # ensure st is in scope for later
    pill_label = "Referral / No-Referral"
    if "MASTER_SECTIONS" in globals() and isinstance(MASTER_SECTIONS, dict):
        perf_list = list(MASTER_SECTIONS.get("Performance", []))
        # insert near the end, but before any 'Sales Activity'/'Call Talk-time' if present
        preferred_after = None  # keep order stable if unknown
        if pill_label not in perf_list:
            insert_idx = len(perf_list)
            for cand in ["Sales Activity", "Sales Activity (Performance)", "Leaderboard", "Performance Overview"]:
                if cand in perf_list:
                    insert_idx = perf_list.index(cand)
                    break
            perf_list.insert(insert_idx, pill_label)
            MASTER_SECTIONS["Performance"] = perf_list
except Exception:
    pass


# ---- Fallback hook: show an entry inside Performance if nav system differs ----
try:
    import streamlit as st
    _master = master if 'master' in globals() else st.session_state.get('nav_master', 'Performance')
    _view = st.session_state.get('nav_sub', '')
    # If we're in Performance but the pill isn't visible/selected, expose a small shortcut
...
# ---- Invoke the new pill when selected ----
try:
    if 'master' in globals():
        _master = master
    else:
        _master = st.session_state.get('nav_master', 'Performance')
    _view = st.session_state.get('nav_sub', '')
    if _master == "Performance" and _view == "Referral / No-Referral":
        # column mapping already exists in the app; reuse globals where possible
