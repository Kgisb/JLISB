...
def _render_performance_activity_tracker(
    ...
    # Determine slider bounds from Last Activity Date (ignore NaT for bounds)
    min_last = pd.to_datetime(d["_last"].min(skipna=True))
    max_last = pd.to_datetime(d["_last"].max(skipna=True))
    # Fallbacks if all NaT
    if pd.isna(min_last) or pd.isna(max_last):
        min_last = pd.to_datetime("2000-01-01")
        max_last = pd.to_datetime(date.today())

    # "Not done since" cutoff slider
    cutoff = st.slider(
        "Not done since (cutoff date) — includes rows with Last Activity Date on/before this date or never done",
    ...
    # Build mask: last activity <= cutoff OR no last activity at all
    mask_last = (d["_last"].dt.date <= cutoff) | (d["_last"].isna())

    # MTD vs Cohort:
    # - Cohort: keep mask_last only
    # - MTD: additionally require Create Date month/year == cutoff month/year
    ...
    d = d[mask].copy()
    if d.empty:
        st.info("No rows match the selected **Not done since** cutoff and mode.")
        return
