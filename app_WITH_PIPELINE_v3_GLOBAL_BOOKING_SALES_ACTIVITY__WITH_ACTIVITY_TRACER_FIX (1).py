...
# ===== Local helpers for Activity tracer =====
def _ac_to_dt(series, dayfirst=True):
    import pandas as pd
    try:
...
def _ac_to_int(series):
    import pandas as pd
    return pd.to_numeric(series, errors="coerce").fillna(0).astype(int)

def _ac_month_bounds(d):
    from datetime import date
    from calendar import monthrange
    start = date(d.year, d.month, 1)
    end = date(d.year, d.month, monthrange(d.year, d.month)[1])
...
def render_activity_tracer(df_scope: pd.DataFrame):
    ...
    today = date.today()
    m_start, m_end = _ac_month_bounds(today)
    ...
    if num_times_contact_col or num_sales_act_col:
        with st.expander("Optional range filters", expanded=False):
            if num_times_contact_col:
                col_ntc = _ac_to_int(df_scope[num_times_contact_col])
            ...
            if num_sales_act_col:
                col_nsa = _ac_to_int(df_scope[num_sales_act_col])
    ...
    d = df_scope.copy()
    d["_create"] = _ac_to_dt(d[create_col]).dt.date
    d["_last"]   = _ac_to_dt(d[last_activity_col]).dt.date

    if num_times_contact_col:
        d["_ntc"] = _ac_to_int(d[num_times_contact_col])
    else:
        d["_ntc"] = 0
    if num_sales_act_col:
        d["_nsa"] = _ac_to_int(d[num_sales_act_col])
    else:
        d["_nsa"] = 0
    ...
    if mode == "MTD":
        ms, me = _ac_month_bounds(end_d)  # MTD relative to end of selected range
...
# Activity tracer minimal router
if master == "Performance" and view == "Activity tracer":
    render_activity_tracer(df_f)
