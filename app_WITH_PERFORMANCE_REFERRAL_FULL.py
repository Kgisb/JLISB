...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Activity Tracker", "Deal stage", "Original source"],
...
# ====== APPENDIX: Performance ▸ Referral ↔ No-Referral (Non-invasive) ======
try:
    import streamlit as st
    import pandas as pd
    import numpy as np
...
def _jlref_find_col(df, candidates):
    cols = {c.lower(): c for c in df.columns}
    for cand in candidates:
        for c in cols:
            if cand.lower() == c: return cols[c]
    for cand in candidates:
        for c in cols:
            if cand.lower() in c: return cols[c]
    return None

def _jlref_normalize_sources(s):
    s = (str(s) if pd.notna(s) else "").strip().lower()
    is_ref = ("refer" in s)
    if "search" in s or "pm-search" in s:
        bucket = "PM-Search"
    elif "social" in s or "pm-social" in s or "facebook" in s or "instagram" in s:
        bucket = "PM-Social"
    elif is_ref:
        bucket = "Referral"
    else:
        bucket = "Other"
    return is_ref, bucket

def _jlref_sales_generated_subset(s):
    t = (str(s) if pd.notna(s) else "").strip().lower()
    return ("sales" in t and "generat" in t)

def _jlref_date_preset_to_range(preset):
    today = date.today()
    if preset == "Today":
        start = datetime(today.year, today.month, today.day); end = start + timedelta(days=1)
    elif preset == "Yesterday":
        end = datetime(today.year, today.month, today.day); start = end - timedelta(days=1)
    elif preset == "This Month":
        start = datetime(today.year, today.month, 1)
        end = datetime(today.year + (1 if today.month==12 else 0), 1 if today.month==12 else today.month+1, 1)
    elif preset == "Last Month":
        start = datetime(today.year-1, 12, 1) if today.month==1 else datetime(today.year, today.month-1, 1)
        end = datetime(today.year, today.month, 1)
    else:
        start = None; end = None
    return start, end

def _jlref_count_in_range(series_dt, start_dt, end_dt):
    if series_dt is None: return 0
    m = (series_dt >= pd.to_datetime(start_dt)) & (series_dt < pd.to_datetime(end_dt))
    return int(m.sum())

def _jlref_timeseries_counts(series_dt, start_dt, end_dt, freq="D"):
    if series_dt is None: return pd.Series(dtype=int)
    idx = pd.date_range(pd.to_datetime(start_dt), pd.to_datetime(end_dt) - pd.Timedelta(seconds=1), freq=freq)
    s = pd.Series(0, index=idx)
    vc = series_dt[(series_dt >= pd.to_datetime(start_dt)) & (series_dt < pd.to_datetime(end_dt))].dt.floor(freq).value_counts().sort_index()
    s.loc[vc.index] = vc.values
    return s

def _jlref_bubble_plot(labels, counts, title):
    fig = plt.figure(figsize=(7,5))
    x = np.arange(len(labels))
    sizes = np.array(counts, dtype=float); sizes = np.where(sizes<=0, 1.0, sizes)
    area = (3000 * (sizes / (sizes.max() if sizes.max() else 1.0)))
    plt.scatter(x, counts, s=area)
    for i,(lbl,val) in enumerate(zip(labels, counts)):
        plt.text(i, val, f"{val}", ha="center", va="bottom", fontsize=9)
    plt.xticks(x, labels, rotation=20, ha="right")
    plt.title(title); plt.ylabel("Count"); plt.tight_layout()
    return fig

def _jlref_line_plot(ts_a, ts_b, title, label_a="Referral (A)", label_b="No-Referral (B)"):
    fig = plt.figure(figsize=(8,4.5))
    ts_a = ts_a.sort_index(); ts_b = ts_b.sort_index()
    plt.plot(ts_a.index, ts_a.values, label=label_a)
    plt.plot(ts_b.index, ts_b.values, label=label_b)
    plt.title(title); plt.xlabel("Date"); plt.ylabel("Count"); plt.legend(); plt.tight_layout()
    return fig

def render_performance_referral_compare(df_base: pd.DataFrame, section_anchor: str = "performance_referral_compare"):
    st.markdown(f'<div id="{section_anchor}"></div>', unsafe_allow_html=True)
    st.markdown("## Performance ▸ Referral ↔ No-Referral")

    df = df_base.copy()

    col_create = _jlref_find_col(df, ["Create Date","Created Date","Deal Create Date"])
    col_pay = _jlref_find_col(df, ["Payment Received Date","Enrolment Date","Enrollment Date"])
    col_trial_sched = _jlref_find_col(df, ["First Calibration Scheduled Date","Trial Scheduled","Trial Schedule Date"])
    col_trial_resched = _jlref_find_col(df, ["Calibration Rescheduled Date","Trial Rescheduled","Trial Reschedule Date"])
    col_trial_done = _jlref_find_col(df, ["Calibration Done Date","Trial Done Date","Trial Completed"])
    col_source = _jlref_find_col(df, ["JetLearn Deal Source","Deal Source","Source"])
    col_ref_intent = _jlref_find_col(df, ["Referral Intent Source","Referral Intent","Referral Source Intent"])
    col_ac = _jlref_find_col(df, ["Academic Counsellor","Academic Counselor","Counsellor","Counselor"])
    col_country = _jlref_find_col(df, ["Country"])

    df["_create_dt"] = _jlref_coerce_datetime(df[col_create]) if col_create else None
    df["_pay_dt"] = _jlref_coerce_datetime(df[col_pay]) if col_pay else None
    df["_trial_sched_dt"] = _jlref_coerce_datetime(df[col_trial_sched]) if col_trial_sched else None
    df["_trial_resched_dt"] = _jlref_coerce_datetime(df[col_trial_resched]) if col_trial_resched else None
    df["_trial_done_dt"] = _jlref_coerce_datetime(df[col_trial_done]) if col_trial_done else None

    ref_flags, buckets = [], []
    src_series = df[col_source] if col_source else pd.Series([None]*len(df))
    for s in src_series:
        r, b = _jlref_normalize_sources(s); ref_flags.append(r); buckets.append(b)
    df["_is_referral"] = ref_flags; df["_bucket"] = buckets

    if df["_create_dt"] is None:
        st.error("Could not find 'Create Date' column for Referral pill."); return

    c1,c2,c3,c4 = st.columns([1.2,1.2,1.2,1.2])
    with c1:
        counting_mode = st.radio("Counting Mode", ["MTD","Cohort"], horizontal=True, key="refcmp_mode_full")
    with c2:
        preset = st.selectbox("Date Preset", ["Today","Yesterday","This Month","Last Month","Custom"])
    with c3:
        group_by = st.selectbox("Optional Group-by", ["(None)","Academic Counsellor","Country"])
    with c4:
        metric_for_trend = st.selectbox("Trend Metric", ["Leads","Trial Scheduled","Trial Rescheduled","Trial Done","Referrals Created","Sales-Generated Referrals","Enrollments"])

    start_dt, end_dt = _jlref_date_preset_to_range(preset)
    if preset == "Custom":
        from datetime import date as _d, timedelta as _td
        d1, d2 = st.date_input("Select custom range", [_d.today()-_td(days=30), _d.today()])
        if isinstance(d1, (list,tuple)): d1, d2 = d1
        start_dt = datetime(d1.year, d1.month, d1.day)
        end_dt = datetime(d2.year, d2.month, d2.day) + timedelta(days=1)

    denom_start_dt, denom_end_dt = start_dt, end_dt

    st.divider()

    b1,b2 = st.columns([1.2,2])
    with b1:
        st.markdown("#### Window B: Non-Referral comparator")
        b_mode = st.radio("Choose comparator:", ["All non-referrals","Pick subset(s)"], key="b_mode_full")
    with b2:
        pick = []
        if b_mode == "Pick subset(s)":
            pick = st.multiselect("Pick comparator buckets", ["PM-Search","PM-Social","Other"], key="b_pick_full")

    grp_col = None
    if group_by == "Academic Counsellor" and col_ac: grp_col = col_ac
    elif group_by == "Country" and col_country: grp_col = col_country

    df_ref = df[df["_is_referral"] == True].copy()
    df_nonref = df[df["_is_referral"] == False].copy()
    if b_mode == "Pick subset(s)" and len(pick) > 0:
        df_nonref = df_nonref[df_nonref["_bucket"].isin(pick)].copy()

    def compute_counts(sub):
        leads = _jlref_count_in_range(sub["_create_dt"], denom_start_dt, denom_end_dt)
        trial_sched = _jlref_count_in_range(sub["_trial_sched_dt"], start_dt, end_dt)
        trial_resched = _jlref_count_in_range(sub["_trial_resched_dt"], start_dt, end_dt)
        trial_done = _jlref_count_in_range(sub["_trial_done_dt"], start_dt, end_dt)

        if "_is_referral" in sub:
            ref_created = int(((sub["_is_referral"]==True) &
                               (sub["_create_dt"]>=pd.to_datetime(denom_start_dt)) &
                               (sub["_create_dt"]<pd.to_datetime(denom_end_dt))).sum())
        else:
            ref_created = 0

        if (col_ref_intent is not None) and ("_is_referral" in sub):
            mask = (sub["_is_referral"]==True) & \
                   (sub["_create_dt"]>=pd.to_datetime(denom_start_dt)) & \
                   (sub["_create_dt"]<pd.to_datetime(denom_end_dt)) & \
                   (sub[col_ref_intent].apply(_jlref_sales_generated_subset))
            sales_gen = int(mask.sum())
        else:
            sales_gen = 0

        if counting_mode == "MTD":
            if sub["_pay_dt"] is None:
                enroll = 0
            else:
                m = (sub["_pay_dt"]>=pd.to_datetime(start_dt)) & (sub["_pay_dt"]<pd.to_datetime(end_dt))
                m &= (sub["_create_dt"]>=pd.to_datetime(denom_start_dt)) & (sub["_create_dt"]<pd.to_datetime(denom_end_dt))
                enroll = int(m.sum())
        else:
            enroll = _jlref_count_in_range(sub["_pay_dt"], start_dt, end_dt)

        conv = round((enroll / leads) * 100, 2) if leads>0 else 0.0
        ...
        return {
            ...
            "Referrals Created": ref_created,
            "Sales-Generated Referrals": sales_gen,
            "Enrollments": enroll,
            "Lead→Enrollment %": conv
        }

    def grouped_or_overall(sub):
        if grp_col is None: return {"(All)": compute_counts(sub)}
        out = {}
        for g, chunk in sub.groupby(grp_col, dropna=False):
            key = str(g) if pd.notna(g) else "(Blank)"
            out[key] = compute_counts(chunk)
        return out

    A = grouped_or_overall(df_ref); B = grouped_or_overall(df_nonref)

    left, right = st.columns(2)
    def show_side(panel_title, data_map):
        st.markdown(f"### {panel_title}")
        for name, metrics in data_map.items():
            st.markdown(f"**{name}**")
            labels = ["Leads","Trial Scheduled","Trial Rescheduled","Trial Done","Referrals Created","Sales-Generated Referrals","Enrollments"]
            counts = [metrics[k] for k in labels]
            fig = _jlref_bubble_plot(labels, counts, title=f"{panel_title} ▸ {name}")
            st.pyplot(fig, clear_figure=True)
            k1,k2,k3,k4 = st.columns(4)
            k1.metric("Leads", metrics["Leads"]); k2.metric("Enrollments", metrics["Enrollments"])
            k3.metric("Trials Done", metrics["Trial Done"]); k4.metric("Lead→Enroll %", f'{metrics["Lead→Enrollment %"]}%')
            st.divider()

    with left: show_side("Window A: Referral", A)
    with right: show_side("Window B: Non-Referral", B)

    def make_ts_for_metric(sub, metric_name):
        if metric_name == "Leads": ser = sub["_create_dt"]
        elif metric_name == "Trial Scheduled": ser = sub["_trial_sched_dt"]
        elif metric_name == "Trial Rescheduled": ser = sub["_trial_resched_dt"]
        elif metric_name == "Trial Done": ser = sub["_trial_done_dt"]
        elif metric_name in ("Referrals Created","Sales-Generated Referrals"):
            ser = sub["_create_dt"]
            if metric_name == "Referrals Created":
                sub = sub[sub["_is_referral"] == True]
            else:
                if (col_ref_intent is not None):
                    sub = sub[(sub["_is_referral"]==True) & (sub[col_ref_intent].apply(_jlref_sales_generated_subset))]
                else:
                    sub = sub.iloc[0:0]
        elif metric_name == "Enrollments":
            ser = sub["_pay_dt"]
            if counting_mode == "MTD":
                sub = sub[(sub["_create_dt"]>=pd.to_datetime(denom_start_dt)) & (sub["_create_dt"]<pd.to_datetime(denom_end_dt))]
        else:
            return pd.Series(dtype=int)
        ser = _jlref_coerce_datetime(ser)
        return _jlref_timeseries_counts(ser, start_dt, end_dt, freq="D")

    st.markdown("### Trend: A vs B")
    tsA = make_ts_for_metric(df_ref.copy(), metric_for_trend)
    tsB = make_ts_for_metric(df_nonref.copy(), metric_for_trend)
    fig_ts = _jlref_line_plot(tsA, tsB, title=f"Trend ▸ {metric_for_trend}")
    st.pyplot(fig_ts, clear_figure=True)

    def dict_to_wide_df(dct, side_label):
        rows = []
        for k, v in dct.items():
            row = {"Group": k, "Side": side_label}; row.update(v); rows.append(row)
        return pd.DataFrame(rows)

    wideA = dict_to_wide_df(A, "Referral (A)")
    wideB = dict_to_wide_df(B, "Non-Referral (B)")
    delta = wideA.merge(wideB, on="Group", how="outer", suffixes=("_A","_B"))
    for col in ["Leads","Trial Scheduled","Trial Rescheduled","Trial Done","Referrals Created","Sales-Generated Referrals","Enrollments"]:
        if f"{col}_A" in delta.columns and f"{col}_B" in delta.columns:
            delta[f"{col} Δ (A-B)"] = delta[f"{col}_A"].fillna(0) - delta[f"{col}_B"].fillna(0)

    cA,cB,cC = st.columns(3)
    with cA:
        st.download_button("Download A (Referral) CSV", data=wideA.to_csv(index=False).encode("utf-8"),
                           file_name="referral_window_A.csv", mime="text/csv", key="dl_A_referral_full")
    with cB:
        st.download_button("Download B (Non-Referral) CSV", data=wideB.to_csv(index=False).encode("utf-8"),
                           file_name="non_referral_window_B.csv", mime="text/csv", key="dl_B_nonref_full")
    with cC:
        st.download_button("Download Δ (A vs B) CSV", data=delta.to_csv(index=False).encode("utf-8"),
                           file_name="referral_compare_delta_A_vs_B.csv", mime="text/csv", key="dl_delta_full")

# Try to locate a main dataframe from the app, else fallback to CSV
try:
    if 'df_f' in globals() and isinstance(df_f, pd.DataFrame): _df_to_use = df_f
    elif 'df' in globals() and isinstance(df, pd.DataFrame): _df_to_use = df
    else: _df_to_use = None
except Exception:
    _df_to_use = None
...
try:
    import streamlit as st
    st.markdown("---")
    if _df_to_use is None:
        _default_csv = "/mnt/data/Master_sheet-DB.csv"
        if os.path.exists(_default_csv):
            _df_to_use = pd.read_csv(_default_csv)
    if _df_to_use is not None:
        render_performance_referral_compare(_df_to_use, section_anchor="performance_referral_compare_bottom")
    else:
        st.info("Referral ↔ No-Referral pill: No dataframe detected in the app. Upload or ensure df_f/df exists.")
except Exception:
    pass
# ====== END APPENDIX ======
