...
def render_activity_tracer(df_scope):
    ...
    # --- Resolve columns from df_scope itself (no globals) ---
    ...
    missing = [k for k, v in needed_map.items() if not v or v not in df_scope.columns]
    if missing:
    ...
    with st.expander("Optional range filters (constrain population)", expanded=False):
        if num_times_contact_col:
            series = to_int(df_scope[num_times_contact_col])
            ntc_range = st.slider("Number of times contacted", min_value=int(series.min()), max_value=int(series.max()), value=(int(series.min()), int(series.max())), key="rng_ntc")
        if num_sales_act_col:
            series2 = to_int(df_scope[num_sales_act_col])
            nsa_range = st.slider("Number of Sales Activities", min_value=int(series2.min()), max_value=int(series2.max()), value=(int(series2.min()), int(series2.max())), key="rng_nsa")
