...
def _agg_side(d0, start_d, end_d, src_pick):
    ...
    # Referral generated = Deal Source contains 'referr' (case-insensitive), counted on CREATED-in-window
    ref_gen = int((m_create & dd["_src"].astype(str).str.contains("referr", case=False, na=False)).sum())
    # Self-generated referrals = Referral Intent Source equals "Sales Generated" (robust normalization), CREATED-in-window
    if "_refi" in dd:
    ...
    _kpi_row("Referral generated (JetLearn Deal Source = referrals)", resA["ReferralGenerated"], resB["ReferralGenerated"])
