...
def _render_performance_original_source(
    ...
    enrol_col: str | None = None,
    drill1_col: str | None = None,
    drill2_col: str | None = None,
):
    import streamlit as st
    import pandas as pd
    import altair as alt
    ...
    if not (drill1_col or drill2_col):
        st.info("No drill-down columns selected. Daily totals will be shown; category views disabled.")
    if not create_col:
        st.warning("Could not find **Create Date** column.")
        return


    # Column selectors (autodetected defaults)
    all_cols = list(df_f.columns)
    def _suggest(name_parts):
        hits = [c for c in all_cols if any(part.lower() in c.lower() for part in name_parts)]
        return hits[0] if hits else None

    default_create = create_col or _suggest(["create","created"])
    default_enrol  = enrol_col or _suggest(["payment received","enrol","enroll","paid"])
    default_d1     = drill1_col or _suggest(["original traffic source drill-down 1","drill down 1","dd1"])
    default_d2     = drill2_col or _suggest(["original traffic source drill-down 2","drill down 2","dd2"])

    create_col = st.selectbox("Create date column", options=["(none)"] + all_cols, index=(all_cols.index(default_create)+1 if default_create in all_cols else 0), key="os_pick_create")
    enrol_col  = st.selectbox("Enrollment date column", options=["(none)"] + all_cols, index=(all_cols.index(default_enrol)+1 if default_enrol in all_cols else 0), key="os_pick_enrol")
    drill1_col = st.selectbox("Drill-Down 1 column", options=["(none)"] + all_cols, index=(all_cols.index(default_d1)+1 if default_d1 in all_cols else 0), key="os_pick_dd1")
    drill2_col = st.selectbox("Drill-Down 2 column", options=["(none)"] + all_cols, index=(all_cols.index(default_d2)+1 if default_d2 in all_cols else 0), key="os_pick_dd2")

    create_col = None if create_col == "(none)" else create_col
    enrol_col  = None if enrol_col  == "(none)" else enrol_col
    drill1_col = None if drill1_col == "(none)" else drill1_col
    drill2_col = None if drill2_col == "(none)" else drill2_col
    # Controls
    mode = st.radio("Counting mode", ["MTD","Cohort"], index=0, horizontal=True, key="os_mode")
    measure = st.radio("Count basis", ["Deals Created (Create Date)","Enrollments (Payment Received Date)","Both (Deals + Enrolments)"], index=0, horizontal=True, key="os_measure")
    ...
    x_axis = st.selectbox("X-axis", ["Date (daily)", "Original Traffic Source Drill-Down 1", "Original Traffic Source Drill-Down 2"], index=0, key="os_xaxis")
    ...
    def _to_dt(s):
        ...
        if preset == "Today":
            ...
            if start > end: start, end = end, start
        # Build basis-specific windows (for "Both")
        start_c, end_c = start, end
        start_e, end_e = start, end
        ...
        # Choose basis (skip assigning _basis when Both; we will filter per-basis below)
        if measure.startswith("Deals"):
            df["_basis"] = df["_create"]
        elif measure.startswith("Enroll"):
            if not enrol_col:
            ...
            df["_basis"] = pd.NaT  # placeholder for "Both"
    
        # Filter by window
        if measure == "Both (Deals + Enrolments)":
            # Deals window
            mask_c = df["_day_create"].between(start_c, end_c)
            # Enrolments window
            mask_e = df["_day_enrol"].between(start_e, end_e)
            if mode == "MTD":
                cohort_ok_c = (df["_create"].dt.month == start_c.month) & (df["_create"].dt.year == start_c.year)
                cohort_ok_e = (df["_enrol"].dt.month == start_e.month) & (df["_enrol"].dt.year == start_e.year)
                mask_c = mask_c & cohort_ok_c.fillna(False)
                mask_e = mask_e & cohort_ok_e.fillna(False)
            df_c = df.loc[mask_c].copy()
            df_e = df.loc[mask_e].copy()
            if df_c.empty and df_e.empty:
                st.info("No rows in the selected windows for **Deals** or **Enrollments**."); return
        else:
        ...
        # Daily distribution / Category totals (with Both support)
        level_col = drill1_col if level.endswith("Down 1") else drill2_col
        if x_axis == "Date (daily)":
            if measure == "Both (Deals + Enrolments)":
                # Build daily totals across all categories for both series
                gd = (df_c.groupby("_day_create").size().rename("Deals Created").reset_index().rename(columns={"_day_create":"_day"})) if not df_c.empty else None
                ge = (df_e.groupby("_day_enrol").size().rename("Enrollments").reset_index().rename(columns={"_day_enrol":"_day"})) if not df_e.empty else None
                from pandas import concat
                if gd is None and ge is None:
                    st.info("No rows in the selected windows for **Deals** or **Enrollments**."); return
                if gd is None:
                    long_df = ge.rename(columns={"Enrollments":"Count"}); long_df["Measure"] = "Enrollments"
                elif ge is None:
                    long_df = gd.rename(columns={"Deals Created":"Count"}); long_df["Measure"] = "Deals Created"
                else:
                    gd = gd.melt(id_vars=["_day"], var_name="Measure", value_name="Count")
                    ge = ge.melt(id_vars=["_day"], var_name="Measure", value_name="Count")
                    long_df = concat([gd, ge], ignore_index=True)
                # KPIs
                total = int(long_df["Count"].sum()); days = long_df["_day"].nunique()
                c1,c2,c3 = st.columns(3)
                with c1: st.metric("Total", f"{total:,}")
                with c2: st.metric("Days", days)
                with c3: st.metric("Avg / day", f"{(total/days if days else 0):.1f}")
                # Chart
                chart_type = st.selectbox("Graph type", ["Line","Bar"], index=0, key="os_chart_both_day")
                base = alt.Chart(long_df).encode(
                    x=alt.X("_day:T", title="Date"),
                    y=alt.Y("Count:Q", title="Count"),
                    color=alt.Color("Measure:N"),
                    tooltip=[alt.Tooltip("_day:T", title="Date"), alt.Tooltip("Measure:N"), alt.Tooltip("Count:Q")]
                ).properties(height=360)
                ch = base.mark_line(point=True) if chart_type == "Line" else base.mark_bar()
                st.altair_chart(ch, use_container_width=True)
                # Table + download
                pivot = long_df.pivot(index="_day", columns="Measure", values="Count").fillna(0).reset_index()
                st.dataframe(pivot, use_container_width=True, hide_index=True)
                csv = pivot.to_csv(index=False).encode("utf-8")
                st.download_button("Download table (CSV)", data=csv, file_name="original_source_both_by_day.csv", mime="text/csv", key="os_dl_both_day")
            else:

                # Existing per-category daily view
                ...
                if level_col and level_col in df.columns:
                    ...
                    # Fallback to totals by day
                    grp = df.groupby(["_day"], dropna=False).size().reset_index(name="Count")
                    grp["__All"] = "All"
                    level_col = "__All"
                # KPIs
            ...
            # Category on X (DD1 or DD2)
            if measure == "Both (Deals + Enrolments)":
                xcol = drill1_col if x_axis.endswith("Down 1") else drill2_col
                # Build totals per category for both series
                ge = df_e.groupby(xcol, dropna=False).size().rename("Enrollments").reset_index() if 'df_e' in locals() and not df_e.empty else None
                gd = df_c.groupby(xcol, dropna=False).size().rename("Deals Created").reset_index() if 'df_c' in locals() and not df_c.empty else None
                from pandas import merge
                if gd is None and ge is None:
                    st.info("No rows for **Deals** or **Enrollments** in selected window."); return
                if gd is None:
                    merged = ge.copy(); merged["Deals Created"] = 0
                elif ge is None:
                    merged = gd.copy(); merged["Enrollments"] = 0
                else:
                    merged = merge(gd, ge, on=xcol, how="outer").fillna(0)
                # Clean categories
                merged[xcol] = merged[xcol].astype(str).replace({'nan':'Unknown','None':'Unknown','NaT':'Unknown'})
                # Top-N by combined total
                merged["Total"] = merged[["Deals Created","Enrollments"]].sum(axis=1)
                merged = merged.sort_values("Total", ascending=False).head(topN).reset_index(drop=True)
                # Build long form for grouped bars
                long_df = merged.melt(id_vars=[xcol], value_vars=["Deals Created","Enrollments"], var_name="Measure", value_name="Count")
                # KPIs
                total = int(long_df["Count"].sum())
                c1,c2 = st.columns(2)
                with c1: st.metric("Total", f"{total:,}")
                with c2: st.metric("Categories", merged.shape[0])
                # Chart
                chart_type = st.selectbox("Graph type", ["Bar","Line"], index=0, key="os_chart_both_cat")
                base = alt.Chart(long_df).encode(
                    x=alt.X(f"{xcol}:N", sort='-y', title=xcol),
                    xOffset=alt.X("Measure:N"),
                    y=alt.Y("Count:Q", title="Count"),
                    color=alt.Color("Measure:N"),
                    tooltip=[alt.Tooltip(f"{xcol}:N", title=xcol), alt.Tooltip("Measure:N"), alt.Tooltip("Count:Q")]
                ).properties(height=360)
                ch = base.mark_bar() if chart_type == "Bar" else base.mark_line(point=True)
                st.altair_chart(ch, use_container_width=True)
                # Table + download
                tbl = merged.drop(columns=["Total"]).rename(columns={xcol: "Original Source Category"})
                tbl.columns = [("Unknown" if (isinstance(c,float) and c != c) else str(c)) for c in tbl.columns]
                st.dataframe(tbl, use_container_width=True, hide_index=True)
                csv = tbl.to_csv(index=False).encode("utf-8")
                st.download_button("Download table (CSV)", data=csv, file_name="original_source_both_by_category.csv", mime="text/csv", key="os_dl_both_cat")
            else:
                # Existing category view
                xcol = drill1_col if x_axis.endswith("Down 1") else drill2_col
                grp = df.groupby(xcol, dropna=False).size().reset_index(name="Count")
                grp[xcol] = grp[xcol].astype(str).replace({'nan':'Unknown','None':'Unknown','NaT':'Unknown'})
