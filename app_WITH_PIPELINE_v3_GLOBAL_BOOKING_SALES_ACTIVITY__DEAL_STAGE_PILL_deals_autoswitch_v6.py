...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Activity Tracker", "Deal stage"],
...
def _render_performance_activity_tracker(
    ...
    with c3:
        ...
        st.metric("Avg / group", f"{avg:.1f}")e 0.0
...
def _render_performance_deal_stage(
    ...
    # Date presets
    today = date.today()
    lbl_basis = "Create Date" if st.session_state.get("ds_metric") == "Deals" else "Last Activity Date"
    preset = st.radio(f"Date range ({lbl_basis})", ["Today","Yesterday","This Month","Custom"], index=2, horizontal=True, key="ds_rng")
    if preset == "Today":
        start, end = today, today
    elif preset == "Yesterday":
        start = today - timedelta(days=1); end = start
    ...
    metric_choice = st.selectbox("Metric", ["Number of times contacted", "Number of Sales Activities", "Deals"], index=0, key="ds_metric")
    ...
    # Base filter mask switches with metric: Deals => Create Date, Others => Last Activity Date
    mask_base = d["_create"].dt.date.between(start, end) if metric_choice == "Deals" and d["_create"].notna().any() else d["_last"].dt.date.between(start, end)
    ...
    if mode == "MTD" and d["_create"].notna().any():
        ...
        mask = mask_base & d["_same_month"].fillna(False)
    else:
        mask = mask_base

    d = d[mask].copy()
    if d.empty:
        st.info("No rows within the selected date window and mode.")
        return
    ...
    last_status = last_status.drop(columns=["Last Activity (max TS)"])    # Metric aggregation
    val_col = None

    # Special case: "Deals" -> count by Create Date within selected window
    if metric_choice == "Deals":
        d_deals = df_f.copy()
        def _to_dt_local(s):
            try:
                return pd.to_datetime(s, errors="coerce", infer_datetime_format=True, dayfirst=True)
                ...
                return pd.to_datetime(pd.Series([None]*len(s)), errors="coerce")
        _create_colname = create_col if (create_col and create_col in d_deals.columns) else None
        if not _create_colname:
            for cand in ["Create Date","Create date","Create_Date","Created At","Deal Create Date","Created On"]:
                if cand in d_deals.columns:
                    _create_colname = cand
                    break
        if not _create_colname:
            st.warning("Could not find **Create Date** to compute Deals. Showing 0s.")
            agg = d.groupby(group_cols, dropna=False).size().reset_index(name="Deals")
        else:
            d_deals["_create"] = _to_dt_local(d_deals[_create_colname])
            mask_create = d_deals["_create"].dt.date.between(start, end)
            d2 = d_deals.loc[mask_create].copy()
            if d2.empty:
                agg = d.groupby(group_cols, dropna=False).size().reset_index(name="Deals")
                agg["Deals"] = 0
            else:
                agg = d2.groupby(group_cols, dropna=False).size().reset_index(name="Deals")
        metric_choice = "Deals"
    ...
    tot = float(out[metric_choice].sum()) if ("out" in locals() or "out" in globals()) and (metric_choice in out.columns) else 0.0
    c1,c2,c3 = st.columns(3)
    with c1: st.metric("Total", f"{int(tot):,}")
