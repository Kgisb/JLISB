...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Activity tracer"],
...
def render_activity_tracer(df_scope):
    import streamlit as st
    import pandas as pd
    import numpy as np
    import altair as alt
    ...
    def to_dt(series, dayfirst=True):
        try:
    ...
    # Resolve columns
    dealstage_col      = find_col(df_scope, ["Deal Stage","Deal stage","Stage","Deal Status","Stage Name","Deal Stage Name"])
    create_col         = find_col(df_scope, ["Create Date","Created Date","Create_Date","Created At","Deal Create Date"])
    last_activity_col  = find_col(df_scope, ["Last Activity Date","Last activity date","Last_Activity_Date"])
    num_sales_act_col  = find_col(df_scope, ["Number of Sales Activities","Number of sales activities","Sales Activities Count","Sales_Activities_Count"])
    num_times_contact_col = find_col(df_scope, ["Number of times contacted","Times Contacted","No. of times contacted","Number_of_times_contacted"])
    counsellor_col     = find_col(df_scope, ["Student/Academic Counsellor","Academic Counsellor","Student/Academic Counselor","Counsellor","Counselor","Deal Owner"])
    country_col        = find_col(df_scope, ["Country","Country Name"])
    source_col         = find_col(df_scope, ["JetLearn Deal Source","Deal Source","Source"])

    # Exclude invalid stage rows
    if dealstage_col:
        mask_invalid = df_scope[dealstage_col].astype(str).str.match(INVALID_RE)
        df_scope = df_scope.loc[~mask_invalid.fillna(False)].copy()

    # Guards
    needed = {"Deal Stage": dealstage_col, "Create Date": create_col, "Last Activity Date": last_activity_col}
    ...
    comp = d.groupby([primary, "Deal Stage"], dropna=False)["_val"].sum().reset_index(name="Value")
    ...
    st.caption(f"Window: **{start_d} → {end_d}** • Mode: **{mode}** • Aggregation: **{agg_title}** • MTD counts only deals whose **Create Date** falls in the same month as the end of the selected range.")




# ---- Activity tracer router (robust) ----
try:
    import streamlit as st
    _is_perf = False
    _is_view = False
    if 'master' in globals():
        _is_perf = (master == "Performance")
    elif st.session_state.get("nav_master"):
        _is_perf = (st.session_state.get("nav_master") == "Performance")
    if 'view' in globals():
        _is_view = (view == "Activity tracer")
    elif st.session_state.get("nav_sub"):
        _is_view = (st.session_state.get("nav_sub") == "Activity tracer")

    if _is_perf and _is_view:
        # Prefer filtered df_f if it exists, else fallback to df
        _df_scope = globals().get("df_f") or globals().get("df") or globals().get("data")  # common names
        if _df_scope is None:
            # last resort: try to find any DataFrame in globals
            _df_scope = next((v for v in globals().values() if hasattr(v, "columns") and hasattr(v, "shape")), None)
        if _df_scope is not None:
            render_activity_tracer(_df_scope)
except Exception as _e:
    try:
        st.error(f"Activity tracer error: {_e}")
    except Exception:
        pass
