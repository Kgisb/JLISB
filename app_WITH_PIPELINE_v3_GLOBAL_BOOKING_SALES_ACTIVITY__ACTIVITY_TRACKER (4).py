...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Activity Tracker"],
...
def _render_performance_activity_tracker(
    ...
    # Date presets
    today = date.today()
    preset = st.radio("Activity date range", ["Today","Yesterday","This Month","Custom"], index=2, horizontal=True, key="act_rng")
    if preset == "Today":
        ...
        c1, c2 = st.columns(2)
        with c1: start = st.date_input("Start", value=today.replace(day=1), key="act_start")
        with c2: end   = st.date_input("End", value=today, key="act_end")
        if start > end:
            start, end = end, start
    ...
    d = d[d["_last"].notna()].copy()
    if d.empty:
        st.info("No rows with a valid Last Activity Date.")
        return

    # Apply range on Last Activity Date
    mask_last = d["_last"].dt.date.between(start, end)

    # MTD vs Cohort logic for activities:
    # - Cohort: count activities with Last Activity Date in range (create anywhere)
    # - MTD:    count activities with Last Activity Date in range AND Create Date in the same month as the window
    ...
    if mode == "MTD" and d["_create"].notna().any():
        # month bounds derived from start/end window
        month_start = start.replace(day=1)
        # if range spans months, consider both month windows covering the whole span
        # for simplicity: require Create Date month==Last Activity Date month for rows within mask_last
        d["_same_month"] = (d["_create"].dt.month == d["_last"].dt.month) & (d["_create"].dt.year == d["_last"].dt.year)
    ...
    d = d[mask].copy()
    if d.empty:
        st.info("No rows after applying the selected window and counting mode.")
        return
    ...
    # Build value to aggregate
    val_col = None
    if metric_choice == "Number of times contacted":
        if times_contacted_col:
            val_col = times_contacted_col
        else:
            st.warning("Column for **Number of times contacted** not found; falling back to counting rows.")
    else:
        if sales_acts_col:
            val_col = sales_acts_col
        else:
            st.warning("Column for **Number of Sales Activities** not found; falling back to counting rows.")

    if val_col and val_col in d.columns:
        # coerce numeric
        vals = pd.to_numeric(d[val_col], errors="coerce").fillna(0)
        d["_val"] = vals
    ...
    # Chart
    chart_df = agg.copy()
    # melt to support stacked bar if multiple dims (but we aggregate single metric per group)
    # Build an x label that concatenates group cols
    ...
    if chart_type == "Bar":
        ...
        # Stacked bar is identical here because there's only one series;
        # kept for UI parity with rest of the app
        ch = base.mark_bar()

    st.altair_chart(ch, use_container_width=True)

    # Table + download
    # Pretty rename group columns
